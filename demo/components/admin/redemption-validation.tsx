...
export function RedemptionValidation({ user }: { user: AdminUser }) {
  ...
  function validate(e: React.FormEvent) {
    ...
    if (match.status !== 'pending') {
      setError(
        `Este canje ya fue ${match.status === 'confirmed' ? 'confirmado' : 'anulado'}. No se puede volver a resolver.`,
      )
      return
    }
    setFound(match)
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      {truncate(reward.description, 90)}
                    </p>
                  </div>
                </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          notify(
                            refund
                              ? 'Canje anulado y puntos devueltos al cliente.'
                              : 'Canje anulado sin devolver puntos.',
                            'info',
                          )
                          reset()
                        }}
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <Badge tone={r.status === 'confirmed' ? 'success' : 'destructive'}>
                    ...
                    <div className="mt-1 grid gap-0.5 text-xs text-muted-foreground">
                      ...
                      {r.status === 'cancelled' ? (
                        <span className={r.pointsRefunded ? 'text-success' : 'text-destructive'}>
                          {r.pointsRefunded
                            ? `Puntos devueltos al cliente (+${r.pointsSpent})`
                            : 'Puntos no devueltos'}
                        </span>
                      ) : null}
                    </div>
