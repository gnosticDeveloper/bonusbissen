...
public record MovementResponse(UUID id, String type, int points, String imagePath, String title, String createdAtFormatted) {
    ...
    public static MovementResponse from(PointTransaction mv) {
        ...
        return new MovementResponse(mv.getId(), mv.getTransactionType().getValue(), mv.getPoints(), mv.getReward().getImagePath(), title, formattedDate);
    }
}
