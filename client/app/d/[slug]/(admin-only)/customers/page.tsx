...
import { parsePositiveInt } from "@/lib/helpers/format";
import { Search, Users } from "lucide-react";
import { CustomerList } from "@/components/customer-list";
import { PageText } from "@/components/customer-page-text";
import { getAllCustomers } from "../../[slug]/points/actions";
...
export default async function CustomersManagerPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | undefined }> }) {
  ...
  const { items: filtered, totalElements } = await getAllCustomers(search ?? "", page, 10);
